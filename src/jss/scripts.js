...
function loadAudio() {
    audioLoader.load('./static/beats/audiofile.mp3?timestamp=' + new Date().getTime(), function(buffer) {
...
function sendData() {
    ...
    var inputValue = inputElement.value.trim().substring(0, 100); // Get the value from the input and trim whitespace, cut to 100 chars
...
function handleServerResponse(response) {
    if (response.action === 'reload_audio') {
        loadAudio();
    }
}
